...
## Task 8: 五险费率与实际申报基数自动测算

Files:
- Create: `app/contribution-calculator.mjs`
- Modify: `app/page.tsx`
- Modify: `app/globals.css`
- Modify: `tests/guided-flow.test.mjs`
...
- 五项默认参考费率合计为 28.9%。
- 应缴基数 20,000 元、实际申报基数 4,986 元时，应缴 5,780 元、实缴 1,440.954 元、差额 4,339.046 元。
- 引导页不再要求用户填写社保公司实际月缴金额，改为实际申报基数并展示五项费率。
- 旧数据只有实际月缴金额时可反推等值申报基数。

Step 2: Run test and confirm failure

Command: `node --test tests/guided-flow.test.mjs`

Expected: 计算模块与新字段尚不存在，测试失败。

Step 3: Implement minimum behavior

- 新增纯计算函数，统一生成应缴、实缴和差额。
- 保留逐月行中的 `socialPaid`、`socialBase`、`socialRate`，使精算台账与旧备份继续兼容。
- 五项费率全部可修改，并明确截图比例只是参考值。

Step 4: Run verification

Command: `npm test && npm run build:pages && git diff --check`

Expected: 纯计算、引导流程、旧数据兼容、服务端渲染和两类生产构建全部通过。

Step 5: Commit

Suggested message: `Calculate social gaps from five-insurance bases`

## Task 9: 报销欠款与 A4 报告导出
...
- 新增 `reimbursement` 事项及金额、说明、计入合计状态。
- 报销只在 `reimbursementIncluded` 为真时进入总计。
- 页面包含独立 A4 报告、报告编号、分项、报销口径、双倍工资依据和免责声明。
- 导出按钮调用 `window.print()`；打印 CSS 隐藏应用界面并只显示报告。

Step 2: Run test and confirm failure

Command: `node --test tests/guided-flow.test.mjs tests/rendered-html.test.mjs`

Expected: 报销字段、报告模板和打印样式尚不存在，测试失败。

Step 3: Implement minimum behavior

- 报销模块默认计入合计，允许切换为仅记录；不引入附件上传。
- 使用 CSS 构建黑白高对比 A4 报告，保留清晰数字层级、点线分项和打印分页控制。
- JSON 升级为版本 7，并兼容旧案例。
...
Expected: 报销口径、报告 SSR、打印样式、旧数据兼容和两类生产构建全部通过。

Step 5: Commit

Suggested message: `Add reimbursement claims and printable reports`
